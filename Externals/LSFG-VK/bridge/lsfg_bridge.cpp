...
struct LsfgNxRuntime {
    explicit LsfgNxRuntime(const LsfgNxCreateInfo& info)
        ...
        const lsfgvk::backend::BorrowedDevice borrowed{
            ...
            .pipelineCachePath = std::filesystem::path(
                "/switch/dolphin/Cache/lsfg-vk-pipeline-cache.bin")
        };
    ...
    VkResult present(VkQueue queue, const VkPresentInfoKHR& info) {
         ...
         * previous frame in slot 1. Warm up slot 1 before the first generated frame. */
        ...
        if (realFrameIndex == 0) {
            ...
            slot.pending = true;
            ++realFrameIndex;
            const VkResult result = presentOriginal(
        ...
        /* VI allows only one acquired image here. Put the generated frame into the image already
         * owned by Dolphin, present it, acquire one released image, then restore the real frame. */
...
private:
    ...
    VkResult presentOriginal(VkQueue queue, const VkPresentInfoKHR& original,
            uint32_t imageIndex, VkSemaphore waitSemaphore, bool keepNextChain) const {
