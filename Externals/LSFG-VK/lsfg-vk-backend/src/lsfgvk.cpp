...
void Context::recordFrame(const vk::CommandBuffer& cmdbuf) {
    ...
    // Preserve the public timeline sequence in case the same context is subsequently used through
    // scheduleFrames(), even though the in-process recording path needs no timeline semaphore.
