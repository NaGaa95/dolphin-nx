...
// Vertex array objects are container objects and are not shared between GL contexts.  Pipeline
// linking on a shader-compiler context therefore needs a VAO created by that context rather than
// one of the main context's vertex-format VAOs.
static thread_local GLuint s_shared_context_vao = 0;
                                                        ...
                                                        size_t cache_data_size)
{
  ...
  {
    // We temporarily change the vertex array to the pipeline's vertex format.  This can prevent
    // the NVIDIA OpenGL driver from recompiling on first use.  VAOs are not shared between
    // contexts, so compiler workers bind their own valid placeholder instead of importing the
    // main context's numeric VAO name.
    const GLuint vao = s_is_shared_context ? s_shared_context_vao :
                                             (vertex_format ? vertex_format->VAO :
                                                              s_attributeless_VAO);
    ASSERT(vao != 0);
...
bool SharedContextAsyncShaderCompiler::WorkerThreadInitWorkerThread(void* param)
{
  ...
  // VAOs are explicitly excluded from GL share groups.  Program linking below binds a VAO as a
  // driver optimization, so each compiler context must own a real local object.  Binding a VAO
  // name generated by the main context is GL_INVALID_OPERATION and is fatal in a no-error context
  // because the driver is allowed to omit that validation.
  glGenVertexArrays(1, &s_shared_context_vao);
  if (s_shared_context_vao == 0)
  {
    s_is_shared_context = false;
    context->ClearCurrent();
    return false;
  }
  glBindVertexArray(s_shared_context_vao);
...
void SharedContextAsyncShaderCompiler::WorkerThreadExit(void* param)
{
  ...
  if (s_shared_context_vao != 0)
  {
    glDeleteVertexArrays(1, &s_shared_context_vao);
    s_shared_context_vao = 0;
  }
  s_is_shared_context = false;
  context->ClearCurrent();
  delete context;
